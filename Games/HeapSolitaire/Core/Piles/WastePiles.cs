...
public class WastePiles : BasicMultiplePilesCP<HeapSolitaireCardInfo>
{
    ...
    public void ClearBoard(DeckRegularDict<HeapSolitaireCardInfo> thisCol)
    {
        if (thisCol.Count != 91)
        {
            throw new CustomBasicException("The collection must have 91 cards");
        }
        base.ClearBoard();
        int y = 0;
    ...
    public HeapSolitaireCardInfo GetCard()
    {
        ...
        if (thisCard.Deck == 0)
        {
            throw new CustomBasicException($"There are no cards to get for {PreviousSelected}");
        }
        return thisCard;
    }
    public bool DidSelectCard => PreviousSelected > -1;
    private readonly HeapSolitaireMainGameClass _mainGame;
    public WastePiles(CommandContainer container,
        HeapSolitaireMainGameClass mainGame) : base(container)
    {
        _mainGame = mainGame;
