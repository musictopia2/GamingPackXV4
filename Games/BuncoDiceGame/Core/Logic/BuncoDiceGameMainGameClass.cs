namespace BuncoDiceGame.Core.Logic;
[SingletonGame]
public class BuncoDiceGameMainGameClass : IAggregatorContainer
{
    ...
    public async Task RollDiceAsync()
    {
        if (_cup == null)
        {
            throw new CustomBasicException("Cup cannot be null.  This means was never loaded.  Rethink");
        }
    ...
    private int CalculateOpponentScore(int whatPlayer)
    {
        ...
        throw new CustomBasicException("Could not find an opponent for player " + whatPlayer);
    }
    ...
    private bool HasWon(int whatPlayer)
    {
        ...
        tempPlayer.WinDetermined = false;
        return false;
    }
    ...
    {
        _global.IsActive = false;
        await Aggregator.PublishAsync(new BuncoNewRoundEventModel());
    }
    ...
    private void ProcessTurnOrder()
    {
        ...
        int currentteam;
        int currenttable;
        CurrentPlayer = _saveRoot!.PlayerList.GetSelf();
        ...
        for (x = 1; x <= 3; x++)
        {
            for (y = 1; y <= 4; y++)
            {
                ...
                {
                    throw new CustomBasicException("This has a duplicate.  Player is " + CurrentPlayer.Id);
                }
                CurrentPlayer.PlayerNum = CurrentPlayer.Id;
                if (currentteam == 1)
                {
                    currentteam = 2;
                }
    ...
    private bool GroupAcceptable()
    {
        ...
        foreach (var tempPlayer in _saveRoot!.PlayerList)
        {
            mate = FindTeamPlayer(tempPlayer.Id, true);
    ...
    private bool AcceptPlacement(int table, int team, bool firstMove, int oldMate = -1)
    {
        ...
        if (firstMove == false)
        {
            if (oldMate == -1)
            {
                throw new CustomBasicException("Need to know the old team mate since this is not the first move");
            }
            return true;
        };
    ...
    private void HumanLists()
    {
        ...
        CurrentPlayer = _saveRoot.PlayerList[self];
        _saveRoot.ThisStats.TeamMate = CurrentPlayer.NickName;
        _saveRoot.ThisStats.YourTable = CurrentPlayer.Table;
        int whattable;
        int whatteam;
