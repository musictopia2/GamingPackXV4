namespace CribbagePatience.Core.Logic;
[SingletonGame]
public class CribbagePatienceMainGameClass : RegularDeckOfCardsGameClass<CribbageCard>, IAggregatorContainer
{
    ...
    internal BasicList<CribbageCombos> ComboList { get; set; } = new();
    internal bool GameGoing { get; set; }
    private RummyProcesses<EnumSuitList, EnumRegularColorList, CribbageCard>? _rummys;
    ...
    public Task NewGameAsync(CribbagePatienceMainViewModel model)
    {
        ...
        if (_rummys != null)
        {
            throw new CustomBasicException("Rummy should not have been created since its fresh every new game");
        }
        _rummys = new();
    ...
    {
        throw new CustomBasicException("Use new function");
    }
    ...
    public void NewRound() //autoresume can't work here unfortunately.  because the override for aftershuffle is not async.
    {
        ...
        if (_model.DeckPile!.IsEndOfDeck())
        {
            throw new CustomBasicException("Can't goto a new round because there are no cards in the deck");
        }
        var tempList = _model.HandScores.Where(items => items.HandCategory != EnumHandCategory.Crib).ToBasicList();
    ...
    public EnumHandCategory CalculateCurrentHandCategory()
    {
        ...
        {
            throw new CustomBasicException("Must have a list of 3 hands.  Otherwise, will be unable to calculate the category");
        }
    ...
    public void RemoveTempCards(IDeckDict<CribbageCard> ThisList)
    {
        ...
        throw new CustomBasicException("Error removing temp cards.  Think about what message to display.");
    }
    public void CardsToCrib(DeckRegularDict<CribbageCard> thisList)
    {
        if (thisList.Count != 2)
        {
            throw new CustomBasicException("Must have 2 cards for crib");
        }
        var cribList = GetCards(EnumHandCategory.Crib);
        if (cribList.Count == 4)
        {
            throw new CustomBasicException("There are already 4 cards to the crib");
        }
        ...
        if (startList.Count == 0)
        {
            ...
            if (tempList.Count != 4)
            {
                throw new CustomBasicException("Should now be 4 cards left for the cribbage list for hand 1");
            }
        ...
        if (startList.Count == 2)
        {
            ...
            if (tempList.Count != 4)
            {
                throw new CustomBasicException("Should now be 4 cards left for the cribbage list for hand 2");
            }
            _model.StartPile!.AddCard(_model!.DeckPile!.DrawCard());
            ResultsFromHandOrCrib();
            return;
        }
        throw new CustomBasicException("Must have 0 or 2 cards left");
    }
    ...
    private BasicList<CribbageCombos> ListCribbageCombos(IDeckDict<CribbageCard> thisCol, bool fromCrib)
    {
        ...
        ComboList.ForEach(thisCombo =>
        {
            ...
            {
                if (thisCol.Any(items => items.Value == EnumRegularCardValueList.Jack && items.Suit == startCard.Suit && startCard.Value != EnumRegularCardValueList.Jack))
                {
                    output.Add(thisCombo);
                }
            }
            ...
            {
                if (thisCol.Any(items => items.Suit == startCard.Suit && startCard.Value == EnumRegularCardValueList.Jack))
                {
                    output.Add(thisCombo);
                }
            }
    ...
    private bool HadFullHouse(IDeckDict<CribbageCard> thisCol)
    {
        ...
        var whatNewRummy = _rummys!.WhatNewRummy(newCol, 3, EnumRummyType.Sets, false);
