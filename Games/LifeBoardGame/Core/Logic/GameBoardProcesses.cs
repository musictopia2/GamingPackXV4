...
public class GameBoardProcesses : ISpacePosition
{
    ...
    private readonly BasicList<ShortcutInfo> _shortCutList = new();
    private readonly BasicList<int> _stops = new();
    public GameBoardProcesses(LifeBoardGameGameContainer gameContainer)
    {
        _gameContainer = gameContainer;
        _stops = new() { 25, 36, 81, 147 };
        _shortCutList = new()
        {
    ...
    public bool CheckMove(int positionClicked)
    {
    ...
    public static EnumViewCategory GetViewForPosition(int currentPosition)
    {
    ...
    public int PlayerInLead
    {
        get
        {
            var tempList = _gameContainer.PlayerList!.OrderByDescending(items => items.Distance).Take(2).ToBasicList();
    ...
    public async Task AnimateMoveAsync(bool useSecond)
    {
        ...
        if (NewPosition > 147)
        {
            throw new CustomBasicException("The highest supported for new position is 147");
        }
        SpaceInfo thisSpace;
        ...
        do
        {
            ...
            if (x > 16)
            {
                throw new CustomBasicException("Can't move more than 16 spaces.  This means its hosed.");
            }
            ...
            {
                throw new CustomBasicException("Current position cannot be 0 if not choosing college or career");
            }
