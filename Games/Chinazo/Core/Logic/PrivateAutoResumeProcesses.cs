...
public class PrivateAutoResumeProcesses(IPrivateSaveGame save) : ISerializable
{
    ...
    public async Task<bool> HasAutoResumeAsync(ChinazoGameContainer container)
    {
    ...
    public async Task RestoreStateAsync(ChinazoGameContainer container)
    {
        if (string.IsNullOrWhiteSpace(_resumeData))
        {
    ...
    private static BasicList<OrganizeModel> GetSaveTempSets(ChinazoVMData model)
    {
    ...
    public async Task SaveStateAsync(ChinazoVMData model)
    {
