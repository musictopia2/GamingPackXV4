...
{
    private BasicList<MoveInfo> PreviousMoves
    {
        get
        {
    ...
    public async Task MakeMoveAsync(int index)
    {
        ...
        {
            _graphicsBoard = _gameContainer.Resolver.Resolve<GameBoardGraphicsCP>();
        }
        _currentSpace = index;
        if (WhoTurn > _gameContainer.PlayerList!.Count)
        {
            throw new CustomBasicException("Number cannot be higher than the total available players");
        }
        _gameContainer.SingleInfo!.PieceList.RemoveSpecificItem(_gameContainer.SaveRoot!.PreviousSpace);
        ...
        _gameContainer.SingleInfo.PieceList.Add(index);
    ...
    private BasicList<MoveInfo> PossibleMovesWithPiece(SpaceInfo thisSpace)
    {
    ...
    private bool DidHave(BasicList<MoveInfo> thisCol, int whatItem)
    {
        if (_gameContainer.SaveRoot!.PreviousSpace == 0)
        {
    ...
    private bool CanWin()
    {
        ...
        var thisCol = _spaceList!.Values.Where(items => items.Player == WhoTurn).ToBasicList();
        EnumColorChoice ourColor;
        ourColor = _gameContainer.SingleInfo!.Color;
    ...
    {
        _gameContainer.Animates!.LongestTravelTime = 80;
        PopulateSpaceList();
    }
    public async Task UnselectPieceAsync()
    {
        _gameContainer.SaveRoot!.PreviousSpace = 0;
        _gameContainer.Aggregator.RepaintBoard();
        await _gameContainer.ContinueTurnAsync!.Invoke();
