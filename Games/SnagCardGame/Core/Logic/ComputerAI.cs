...
public static class ComputerAI
{
    public static int CardToPlay(SnagCardGameMainGameClass mainGame, SnagCardGameVMData model)
    {
        if (mainGame.SaveRoot!.FirstCardPlayed == false)
        {
            ...
            if (thisCol.Count == 0)
            {
                throw new CustomBasicException("No cards left from bar");
            }
            if (thisCol.Count > 2)
            {
                throw new CustomBasicException("There cannot be more than 2 cards to choose from for bar for computer player");
            }
        ...
        {
            throw new CustomBasicException("There cannot be 0 cards left");
        }
