...
public class TriangleBoard : TriangleObservable
{
    ...
    public void ClearCards(IEnumerable<SolitaireCard> thisCol)
    {
        if (thisCol.Count() != 28)
        {
            throw new CustomBasicException("Must have 28 cards to place here");
        }
    ...
    public void MakeInvisible(int deck)
    {
        ...
        {
            throw new CustomBasicException("There can only be 2 images selected");
        }
        if (FirstDeck == 0)
        {
            FirstDeck = deck;
    ...
    public override void LoadSavedTriangles(SavedTriangle thisT)
    {
        ...
        {
            throw new CustomBasicException("After loading saved game, must have 28 cards");
        }
    }
    public void PutBackOne(int deck)
    {
        if (FirstDeck == 0 && SecondDeck == 0)
        {
            throw new CustomBasicException("There was nothing previously selected at all");
        }
        SolitaireCard thisCard;
        ...
        if (FirstDeck > 0 && SecondDeck > 0)
        {
            ...
            {
                throw new CustomBasicException($"There was nothing previously selected with the deck of {deck}");
            }
            if (FirstDeck == deck)
            {
                FirstDeck = 0;
        ...
        if (FirstDeck > 0)
        {
            if (FirstDeck != deck)
            {
                throw new CustomBasicException($"There is no deck found for {deck}");
            }
            FirstDeck = 0;
            RecalculateEnables();
            return;
