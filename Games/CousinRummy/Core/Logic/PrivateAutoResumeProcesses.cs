...
public class PrivateAutoResumeProcesses(IPrivateSaveGame save) : ISerializable
{
    ...
    public async Task<bool> HasAutoResumeAsync(CousinRummyGameContainer container)
    {
    ...
    public async Task RestoreStateAsync(CousinRummyGameContainer container)
    {
        if (string.IsNullOrWhiteSpace(_resumeData))
        {
    ...
    private static BasicList<OrganizeModel> GetSaveTempSets(CousinRummyVMData model)
    {
    ...
    public async Task SaveStateAsync(CousinRummyVMData model)
    {
