...
public class MoveProcesses : IMoveProcesses
{
    ...
    private async Task MonthProcessingAsync()
    {
        ...
        if (_gameContainer.SaveRoot.RemainingMove > 0)
        {
            ...
            {
                throw new CustomBasicException("Cannot be any remaining move because the player is no longer in the game");
            }
    ...
    private static int Thousands(decimal amount)
    {
        int x = default;
        if (amount < 1000)
            return 0;
    ...
    private void PayLoans()
    {
        ...
        {
            throw new CustomBasicException("The loans and money a player has must be greater or equal to 0");
        }
    }
    ...
    private void ProcessBirthday()
    {
        ...
        BasicList<PaydayPlayerItem> tempList = _gameContainer.PlayerList!.Where(items => items.Id != _gameContainer.WhoTurn).ToBasicList();
    ...
    public async Task ResultsOfMoveAsync(int day)
    {
        ...
        switch (details)
        {
            ...
            case EnumDay.WalkForCharity:
                ...
                _gameContainer.SaveRoot.Instructions = "Walk for charity.  Roll the dice and pay 100 times the amount you roll to the jackpot";
                _gameContainer.Command.UpdateAll();
                await Pause3Async();
                if (_gameContainer.OtherTurnProgressAsync == null)
                {
                    throw new CustomBasicException("Nobody is handling other turn progress.  Rethink");
                }
                await _gameContainer.OtherTurnProgressAsync.Invoke();
                break;
            default:
                throw new CustomBasicException($"Don't know what to do with {_gameContainer.SaveRoot.GameStatus}");
        }
    }
