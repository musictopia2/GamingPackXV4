...
public class MailCard : CardInformation
{
    ...
    public override void Populate(int chosen)
    {
        ...
        if (oldDeck <= 0)
        {
            throw new CustomBasicException("Must be 1 to 47.  If 0 is allowed; then I need to return new mail card");
        }
        Deck = chosen;
        Index = oldDeck;
        switch (oldDeck)
        {
            ...
            case 47:
                ...
                break;
            default:
                throw new CustomBasicException($"Deck must be between 1 and 47 for the mail, not {oldDeck}");
        }
    }
}