...
public class DealCard : CardInformation
{
    ...
    public override void Populate(int chosen)
    {
        if (chosen == 0)
        {
            throw new CustomBasicException("Must be between 1 And 24.  If 0 Is allowed; then I need to return a New deal card");
        }
        Deck = chosen;
        Index = Deck;
        switch (chosen)
        {
            ...
            case 24:
                ...
                break;
            default:
                throw new CustomBasicException($"Deck must be between 1 and 24 for the deal, not {chosen}");
        }
    }
}