...
public class CandylandCardData : SimpleDeckObject, IDeckObject
{
    ...
    {
        DefaultSize = new SizeF(150, 84);
    }
    public EnumCandyLandType WhichCard { get; set; } = EnumCandyLandType.None;
    ...
    private static int FindIndex(int chosen)
    {
        ...
        throw new CustomBasicException("Cannot find index for " + chosen);
    }
    public override string ToString()
    {
        if (HowMany > 0)
        {
            return $"Deck: {Deck}. {HowMany} Of {WhichCard} ";
        }
        else
        {
            return $"Deck: {Deck}.  {WhichCard}";
        }
    }
    public void Populate(int chosen)
    {
        if (chosen == 0)
        {
            throw new CustomBasicException("Item chosen cannot be 0");
        }
        Deck = chosen;
