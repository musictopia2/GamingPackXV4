...
public class UnoComputerAI
{
    ...
    private DeckRegularDict<UnoCardInformation> ValidMoveList()
    {
        ...
        {
            throw new CustomBasicException("Nobody is handling the canplay.  Rethink");
        }
    ...
    private int ColorMatches(DeckRegularDict<UnoCardInformation> moveList)
    {
        ...
        tempList.KeepConditionalItems(x => x.Color == _gameContainer.SaveRoot.CurrentColor);
        if (tempList.Count == 0)
        {
            return 0;
        }
    ...
    private int NumberMatches(DeckRegularDict<UnoCardInformation> moveList)
    {
        ...
        tempList.KeepConditionalItems(x => x.Number == currents.Number);
        if (tempList.Count == 0)
        {
            return 0;
        }
    ...
    private static int SpecialCard(DeckRegularDict<UnoCardInformation> moveList, string whatLabel)
    {
        var tempList = moveList.ToRegularDeckDict();
        if (whatLabel == "Draw 2")
        {
        ...
        {
            throw new CustomBasicException($"Cannot find any special card to attempt for {whatLabel}");
        }
        if (tempList.Count == 0)
        {
            return 0;
    ...
    private int MaxPoints(DeckRegularDict<UnoCardInformation> moveList)
    {
        ...
        return moveList.OrderByDescending(x => x.Points).Take(1).Single().Deck;
    }
