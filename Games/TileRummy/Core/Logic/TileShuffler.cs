     ...
     IAdvancedDIContainer, ISimpleList<TileInfo>, IListShuffler<TileInfo>
{
    ...
    private readonly DeckRegularDict<TileInfo> _objectList = new();
    ...
    public TileShuffler()
    {
        _thisShuffle = new BasicObjectShuffler<TileInfo>(_objectList);
        NeedsToRedo = true;
    }
    public Task<DeckRegularDict<TileInfo>> GetListFromJsonAsync(string jsonData)
    {
    ...
    public Task ForEachAsync(ActionAsync<TileInfo> action)
    {
    ...
    public IBasicList<TileInfo> FindAll(Predicate<TileInfo> match)
    {
