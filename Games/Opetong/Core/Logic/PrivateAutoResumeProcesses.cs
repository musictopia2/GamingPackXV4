...
public class PrivateAutoResumeProcesses(IPrivateSaveGame save) : ISerializable
{
    ...
    public async Task<bool> HasAutoResumeAsync(OpetongGameContainer container)
    {
    ...
    public async Task RestoreStateAsync(OpetongGameContainer container)
    {
        if (string.IsNullOrWhiteSpace(_resumeData))
        {
    ...
    private static BasicList<OrganizeModel> GetSaveTempSets(OpetongVMData model)
    {
    ...
    public async Task SaveStateAsync(OpetongVMData model)
    {
