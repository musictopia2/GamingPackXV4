...
public class PlayProcesses : IPlayProcesses
{
    ...
    public async Task PlayCardAsync(FluxxCardInformation card)
    {
        ...
        {
            throw new CustomBasicException("Everybody gets one was not finished.  That must be finished before playing another card");
        }
        if (_gameContainer.TempActionHandList.Count > 0)
        {
            throw new CustomBasicException("There are cards left from the temporary action list.  Must finish choosing the order to play the cards.  Then they will play in the order");
        }
        ...
        switch (card.CardType)
        {
            ...
            case EnumCardType.Action:
                ...
                break;
            default:
                throw new CustomBasicException("Can't figure out which one to play for card type");
        }
    }
            ...
            if (_gameContainer.CurrentAction.Category == EnumActionScreen.OtherPlayer)
            {
                ...
                if (tempList.Any(items => items.MainHandList.Count > 0))
                {
                    do
                    {
                        ...
                        var tempPlayer = _gameContainer.PlayerList.GetOtherPlayer();
                        ...
                        {
                            throw new CustomBasicException("There should have been at least one player with cards for taxation");
                        }
                    } while (true);
    ...
    private async Task PlayGoalAsync(GoalCard thisCard)
    {
        ...
        {
            throw new CustomBasicException("Needs to remove a goal before it can play a goal");
        }
        if (_gameContainer.SaveRoot.GoalList.Count == 2 && isDouble == false)
        {
            throw new CustomBasicException("Cannot play another goal because 2 goals already and not double agenda");
        }
    ...
    private async Task PlayKeeperAsync(KeeperCard thisCard)
    {
        if (_gameContainer.OtherTurn > 0)
        {
            throw new CustomBasicException("Can't be otherturn for playing a keeper.  If its playing first card random; will set OtherTurn to 0 first");
        }
        _gameContainer.SingleInfo = _gameContainer.PlayerList!.GetWhoPlayer();
    ...
    async Task IPlayProcesses.PlayUseTakeAsync(int deck, int player)
    {
        if (_gameContainer.SingleInfo!.CanSendMessage(_gameContainer.BasicData!))
        {
            BasicList<int> newList = new() { deck, player };
    ...
    private async Task ScrambleKeepersAsync()
    {
        ...
        {
            var thisList = _gameContainer.PlayerList!.Where(items => items.KeeperList.Count > 0).Select(temps => temps.KeeperList.Select(fins => (int)fins.Deck).ToBasicList().ToBasicList());
            await _gameContainer.Network!.SendAllAsync("scramblekeepers", thisList.ToBasicList());
        }
        await _analyzeProcesses.AnalyzeQueAsync();
    }
    ...
    private async Task DrawTemporaryCardsAsync()
    {
        ...
        if (_gameContainer!.TempActionHandList.Count > 0)
        {
            throw new CustomBasicException("There are already cards left to choose from.  Should have not Drawn Temporary Cards");
        }
    ...
    {
        var tempList = _gameContainer.SaveRoot!.RuleList.Where(items => items.Category == EnumRuleCategory.Hand || items.Category == EnumRuleCategory.Keeper).ToBasicList();
